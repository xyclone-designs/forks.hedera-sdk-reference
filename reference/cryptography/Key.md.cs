...
namespace Hedera.Hashgraph.Reference.Cryptography
{
    ...
    /// 
    /// A IKey can be a smart contract ID, which means that smart contract is to authorize operations as
    ...
    /// 
    /// A IKey can be a "threshold key", which means a list of M keys, any N of which must sign in order
    ...
    /// 
    /// A IKey can be a "key list" where all keys in the list must sign unless specified otherwise in the
    ...
    /// 
    /// A IKey can contain a ThresholdKey or KeyList, which in turn contain a Key, so this mutual
    /// recursion would allow nesting arbitrarily deep. A ThresholdKey which contains a list of primitive
    /// keys (e.g., ed25519) has 3 levels: ThresholdKey -> IKeyList -> Key. A IKeyList which contains
    /// several primitive keys (e.g., ed25519) has 2 levels: IKeyList -> Key. A IKey with 2 levels of
    /// nested ThresholdKeys has 7 levels: IKey -> ThresholdKey -> IKeyList -> IKey -> ThresholdKey ->
    /// IKeyList -> Key.
    /// 
    /// Each IKey should not have more than 46 levels, which implies 15 levels of nested ThresholdKeys.
